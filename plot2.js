...
(function() {
  ...
  Trades.prototype.updateTradeCount = function(quote) {
    ...
    this.tradeCountAllOpen = this.BuyOpen.length + this.SellOpen.length;
  ...
  function strategySimple4(quotes) {
    ...
    trades.updateEquity(quotes[i - 1]);
    console.log("trades closed: " + trades.tradeCountAllClosed);
    return trades;
  }
  function strategySimple4_0(quotes) {
    trades = new Trades();
    let trade = undefined;
    let i = 0;
    for (i = 1; i < quotes.length; i++) {
    ...
    for (i = 1; i < quotes.length; i++) {
      ...
      if (quotes[i] > quotes[i - 1]) {
        ...
        if (trade) {
          ...
          if (trades.tradeCountAllOpen >= tradeCountMax) {
  ...
  function strategySimple4_2(quotes) {
    trades = new Trades();
    let tradeSell = tradeBuy = tradeOpen = undefined;
    let tradeCountMax = 40;
    tradeCount = 0;
    let i = 0;
    for (i = 1; i < quotes.length; i++) {
      ...
      trades.updateEquity(quotes[i]);
      trades.updateTradeCountFullRange();
      tradeSell = tradeBuy = tradeOpen = undefined;
      // buy
      if (quotes[i] > quotes[i - 1]) {
        // tradeOpen = trades.isOpenBuyAt(i, quotes[i]);
        // trades.closeAllOpenTradeIf(i, quotes[i], trades.SellOpen,
        //   trade => trade.getProfitOpen(quotes[i]) == 1);
        // trade = trades.isOpenSellAt(i, quotes[i] + 1);
        // if (trade) {
        //   trades.close(trade, i, quotes[i]);
        // } else {
        // }
      ...
      // sell  
      if (quotes[i] < quotes[i - 1]) {
        // tradeOpen = trades.isOpenSellAt(i, quotes[i]);
        // trades.closeAllOpenTradeIf(i, quotes[i], trades.BuyOpen,
          // trade => trade.getProfitOpen(quotes[i]) == 1);
        // trade = trades.isOpenBuyAt(i, quotes[i] - 1);
        // if (trade) {
        //   trades.close(trade, i, quotes[i]);
        // } else {
        // }
